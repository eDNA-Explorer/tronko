...
- **Serialized lazy index cache** (decision: mutex/`pthread_once`-style). File-scope `static mm_idx_t *g_mm2_idx`, `g_mm2_db_path`, `g_mm2_kmer`, `g_mm2_window`, guarded by a `static pthread_mutex_t`. Every worker locks; the first to enter builds the index from `databaseFile` (the `-a` FASTA) with `iopt.k=mm2_kmer; iopt.w=mm2_window;`, the rest reuse it. Rebuild only if path/k/w change.
