...
void allocateMemForResults( resultsStruct *results, int sizeOfChunk, int num_threads, int numberOfTrees, int print_alignments, int maxNumSpec, int paired, int use_nw, int max_lineTaxonomy, int max_name_length, int max_query_length, int max_numbase, int use_portion, int padding_size, int number_of_total_nodes, int max_leaf_matches){
	...
	if ( use_portion==1){
		results->starts_forward = (int *)malloc(max_leaf_matches*sizeof(int));
		if (paired == 1 ){
			results->starts_reverse = (int *)malloc(max_leaf_matches*sizeof(int));
		}
		results->cigars_forward = (char **)malloc(max_leaf_matches*sizeof(char *));
		if (paired == 1){
			results->cigars_reverse = (char **)malloc(max_leaf_matches*sizeof(char *));
		}
		for(i=0; i<max_leaf_matches; i++){
	...
	results->leaf_coordinates = (int **)malloc(max_leaf_matches*sizeof(int *));
	for(i=0; i<max_leaf_matches; i++){
