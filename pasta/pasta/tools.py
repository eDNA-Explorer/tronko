...
class FastTree(TreeEstimator):
    ...
    def create_job(self, alignment, starting_tree=None, **kwargs):
        ...
        if self._backend == 'veryfasttree':
            # VeryFastTree: single binary with native -threads flag,
            # scales well to all available cores (no 4-thread cap).
            vft_path = shutil.which('VeryFastTree')
            if vft_path is None:
                raise RuntimeError(
                    "PASTA_TREE_BACKEND=veryfasttree but VeryFastTree binary "
                    "not found on PATH. Install it or set PASTA_TREE_BACKEND=fasttree.")
            invoc[0] = vft_path
            if num_cpus and num_cpus > 1:
                invoc.insert(1, '-threads')
                invoc.insert(2, str(num_cpus))
        elif num_cpus > 1:
            # FastTree: use the OpenMP MP variant, capped at 4 threads.
            if platform.system() == 'Windows':
