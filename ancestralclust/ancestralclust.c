...
void setNumSeq(FILE* fasta, int* fasta_specs){
	...
	fasta_specs[3] = minSeq; // set min sequence length
}
...
void *fillInMat(void *ptr){
	...
	int i,j;
	// OPT1: Create aligner once, reuse across all iterations
	wavefront_aligner_attr_t attributes = wavefront_aligner_attr_default;
	attributes.distance_metric = gap_affine;
	attributes.affine_penalties.mismatch =4;
	attributes.affine_penalties.gap_opening = 6;
...
// OPT2: Compute JC distance directly from CIGAR operations in O(L) time.
...
double compute_JC_from_CIGAR(cigar_t* const cigar, char* seq1, char* seq2, int seq1_len, int seq2_len){
	...
	int k;
	for(k = cigar->begin_offset; k < cigar->end_offset; ++k){
		switch(cigar->operations[k]){
			case 'M':
			case 'X': {
				int b1 = encode_base(seq1[pattern_pos++]);
				int b2 = encode_base(seq2[text_pos++]);
				if(b1 > -1 && b2 > -1){
					numrealsites++;
					if(b1 != b2) numdiff++;
				}
	...
	// Trailing endsfree bases (if any) are effectively gaps, not counted
	if(numrealsites < 5) return 1.0;
	if(numdiff == 0) return 0.0;
	double dmax = 0.75*(1.0 - exp(-4.0*DISTMAX/3.0));
	double rawd = (double)numdiff/(double)numrealsites;
	if(rawd >= dmax) return DISTMAX;
	return -0.75*log(1.0 - 4.0*rawd/3.0);
}
double findShortestDist_WFA(int index, char* seq, int clusterSize, double** distMat2, int** DATA, int* mult){
	...
	for(i=0; i<clusterSize; i++){
		...
		distMat2[i][0] = compute_JC_from_CIGAR(wf_aligner->cigar,rootSeqs[index],seq,seq1_len,seq2_len);
	}
	wavefront_aligner_delete(wf_aligner);
...
void *runAssignToCluster(void *ptr){
	...
	// OPT4: Create WFA aligner once per thread, reuse across all sequences+clusters
	wavefront_aligner_attr_t wfa_attributes = wavefront_aligner_attr_default;
	wfa_attributes.distance_metric = gap_affine;
	wfa_attributes.affine_penalties.mismatch = 4;
	wfa_attributes.affine_penalties.gap_opening = 6;
	wfa_attributes.affine_penalties.gap_extension = 2;
	wfa_attributes.alignment_form.span = alignment_endsfree;
	wavefront_aligner_t* thread_wf_aligner = wavefront_aligner_new(&wfa_attributes);
	...
	// OPT4: Destroy thread-local aligner
	wavefront_aligner_delete(thread_wf_aligner);
	free(DATA[0]);
	free(DATA[1]);
	free(DATA);
	free(mult);
...
int main(int argc, char **argv){
	...
	for(i=0; i<kseqs; i++){
		...
		if (foundPath[0]==0){
			addFirstCluster(tree,i,0,parentcuts,kseqs-1,fasta_specs[4]+1,cluster_seqs,index);
		}
	}
	//for(i=0; i<kseqs; i++){
	//	free(clusterarr[i]);
	...
	freeClusters(fasta_specs[4]+1,kseqs,cluster_seqs);
	if (opt.hasTaxFile==1){
