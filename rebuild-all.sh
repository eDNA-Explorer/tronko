...
run_pasta_pipeline() {
    ...
    for variant in $VARIANTS; do
        ...
        local variant_dir="${DB_BASE}/${variant}"
        local pasta_out="${variant_dir}/pasta_output"
        local cache_dir="${variant_dir}/pasta_cache"
        mkdir -p "$cache_dir"
        ...
        for size in "${PASTA_SIZES[@]}"; do
            ...
            local part_dir="${cache_dir}/partitions_${part_label}"
            ...
            for gamma in "${PASTA_GAMMA[@]}"; do
                ...
                local build_dir="${variant_dir}/${part_label}_${suffix}"
                log "    $variant/${part_label}_${suffix} ($n_parts partitions)"
                run_tronko_on_partitions "$part_dir" "$build_dir" "$n_parts" -f 999999
            done

            # SP repartitioning variants
            for sp in "${PASTA_SP_THRESHOLDS[@]}"; do
                local build_dir="${variant_dir}/${part_label}_sp${sp}"
                log "    $variant/${part_label}_sp${sp} ($n_parts partitions)"
...
run_ac_pipeline() {
    ...
    for variant in $VARIANTS; do
        ...
        for ac_entry in "${AC_CONFIGS[@]}"; do
            ...
            log "  AC config: $ac_label (bin=$ac_bin_size, desc=$ac_descendants)"

            for sp in "${AC_SP_THRESHOLDS[@]}"; do
                local label="${ac_label}_sp${sp}"
                local build_dir="${variant_dir}/${label}"

                if [[ -f "$build_dir/reference_tree.txt" ]] || \
                   [[ -f "$build_dir/reference_tree.txt.gz" ]] || \
                   [[ -f "$build_dir/reference_tree.trkb" ]]; then
                    log "    $variant/$label already exists — skipping"
                    continue
                fi

                log "    Building $variant/$label (SP=$sp)..."
